...
export const ChampionAbilities = ({ champ }: { champ: ddChampion }) => {
  ...
  return (
    <Suspense>
      <div tw="grid xs:[grid-template-columns: repeat(auto-fit, minmax(140px, 0.5fr) minmax(140px, 0.5fr))] md:grid-cols-4 lg:gap-8 gap-4 place-content-center text-sm">
        {query.data.data[champ.id].spells.map((s, i) => (
          ...
          >
            <img
              ...
              title={s.description}
            />
            <div tw="grid [grid-template-columns: repeat(auto-fit, auto)] [grid-template-rows: 32px]">
              <div tw="grid lg:gap-2 gap-1 items-center [grid-template-columns: auto 1fr] place-self-stretch justify-items-stretch">
              ...
              <label tw="grid [grid-template-columns: 5ch 1fr] md:gap-1">
                <b tw="text-xs">Cd(s):</b>
              ...
              <label tw="grid [grid-template-columns: 5ch 1fr] justify-items-stretch">
                <b tw="text-xs">Cost:</b>
                <div tw="grid gap-0.5 [grid-template-columns: repeat(auto-fit, minmax(2ch, 3ch))] lg:gap-2 text-xs text-end justify-end">
                  {s.cost.map((c, i) => (
